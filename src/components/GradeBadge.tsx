...
import { GRADE_COLOR, GRADE_KEY, type Grade } from "@/config/indicators";
import { useI18n } from "@/i18n";

export interface GradeBadgeProps {
  grade: Grade;
  /** 등급이 왜 그런지. 툴팁으로 붙는다 (예: 출처명). */
  reason?: string;
  size?: "sm" | "md";
}
...
export default function GradeBadge({ grade, reason, size = "md" }: GradeBadgeProps) {
  ...
  return (
    <span className={className} title={reason ? `${label} — ${reason}` : label} data-grade={grade}>
