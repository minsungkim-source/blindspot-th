...
- [~] **ATM을 넣으면 공급 1위가 방콕에서 푸껫으로 바뀐다** (98.42 vs 98.29).
      → **근거 자료 완성 (2026-09-22)**: `docs/WEIGHT_SENSITIVITY.md` §2.
        격차가 **0.13pt로 사실상 동률**이고, 공급 축에 규모 항(log 지점수)을 **5%만**
        넣으면 1위가 방콕으로 돌아오면서 **우선순위 상위 10이 하나도 안 바뀐다**
        (최대 순위이동 4계단). 10% 이상부터 상위 10이 교체되기 시작한다.
        결정은 세션 몫이지만 선택지의 대가는 이제 측정되어 있다.
...
- [ ] 이해관계자 리뷰 → 기본 가중치 프리셋 확정 (영업·리스크·전략 합동 세션)
      → **준비물 완성 (2026-09-22)**: `docs/WEIGHT_SENSITIVITY.md` + 재실행 스크립트
        `etl/analysis/weight_sensitivity.py`. 세션에서 결정할 것이 셋으로 좁혀졌다
        (규모 항 · 기본 프리셋 · 기본값과 합의가 어긋나는 두 주)
      · **세션을 기다리지 않아도 되는 것**: Chaiyaphum · Narathiwat · Sa Kaeo는
        무작위 가중치 2,000개 중 86% 이상에서 상위 10에 든다. 가중치가 어떻게
        정해지든 상위권이라 지금 착수해도 안전하다
      · **세션이 필요한 이유**: 43/77개 주가 어떤 가중치에서는 상위 10에 든다.
        기본값을 고르는 일이 화면 설정이 아니라 사업 결정이라는 뜻이다
...
## Sprint 6 — 가중치 민감도에서 드러난 것 (2026-09-22)

- [x] `etl/analysis/weight_sensitivity.py` + `docs/WEIGHT_SENSITIVITY.md` —
      프리셋 비교 · 규모 항 스윕 · 앙상블 강건성 · 프리셋 vs 합의.
      `transform.figi`의 primitive를 불러 쓴다 — **산식의 세 번째 사본을 만들지 않는다.**
      `tests/test_weight_sensitivity.py`가 하네스와 커밋된 산출물의 일치를 고정한다
      (이게 깨지면 사이트는 멀쩡한데 분석만 틀린 상태가 된다)
- [x] **우선순위 레이어의 램프가 7단계 중 4단계만 쓰던 것을 고쳤다** (2026-09-23).
      `priority = clip(gap_raw × log10(인구), 최소 0)`이라 **공급이 수요 이상인 40개 주가
      전부 정확히 0**이었고, `scaleQuantile`이 그 덩어리를 한 칸에 몰아넣으면서 경계까지
      끌어당겼다. 결과적으로 **44개 주가 같은 색**이었다 (갭 레이어는 7/7 정상이었다).
      · 정의 자체는 옳다 — '확장 대상 아님'을 0으로 표현한 것. 문제는 **척도 밖의 값에
        척도의 한 칸을 준 것**이었다. 바닥값을 분위수 도메인에서 아예 빼냈다
      · `--seq-floor`(#2E353B)를 새 토큰으로. `--seq-null`(데이터 없음)과 **다른 색**이다 —
        '값을 모른다'와 '값을 알고 그 값이 척도 밖이다'는 다른 말이고, 같은 색이면
        지도가 그 둘을 구분해 말할 수 없다
      · 범례에 별도 칸("확장 대상 아님 (공급 ≥ 수요)") + **양끝 값도 바닥값을 뺀 뒤로**
        (0.0 → 4.6부터). 램프가 표현하지 않는 값을 램프 끝에 적으면 범례가 거짓말이 된다
      · `aria-label`도 숫자가 아니라 범주로 읽는다 ("우선순위 — 확장 대상 아님").
        색만으로 의미를 나르지 않는 규칙을 스크린리더 쪽에도 똑같이 적용했다
      · `forced-colors`에서는 색이 사라지므로 **형태로 세 갈래**: 값 있음 해치 ·
        바닥값 점 · 값 없음 빈 채움. 점 패턴(`fc-dots`)을 새로 넣었다
      · 바닥값은 지표의 성질이라 `MAP_LAYERS`의 `floor`로 선언한다. 갭·지점밀도 등
        그 개념이 없는 레이어는 이전과 완전히 동일하게 동작한다 (회귀 확인)
      · **실측 검증**: 우선순위 레이어 고유 색 4 → **8**(램프 7 + 바닥값),
        37개 대상 주가 6/5/5/5/5/5/6으로 고르게 분포. 갭 레이어는 7색 유지
      · 근거는 `DESIGN.md` §2.3(신설), 수치는 `docs/WEIGHT_SENSITIVITY.md` §5.
        `src/lib/scale.test.ts` 9케이스가 고정한다
- [x] **Sprint 2 검증의 빈틈을 메웠다** — "램프 7단계가 실제로 다 쓰인다"를
      기본 레이어(갭)에서만 확인했던 것이 이 버그를 8개월 가까이 숨겼다.
      이제 레이어별로 세는 테스트가 있다
