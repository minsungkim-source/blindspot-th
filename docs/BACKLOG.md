...
- [ ] **우선순위 레이어의 램프가 7단계 중 4단계만 쓴다.** `docs/WEIGHT_SENSITIVITY.md` §5.
      `priority = clip(gap_raw × log10(인구), 최소 0)`이라 **공급이 수요 이상인 40개 주가
      전부 정확히 0**이고, `scaleQuantile`이 그 40개를 한 버킷에 몰아넣는다.
      결과적으로 우선순위 레이어를 켜면 **44개 주가 같은 색**이다 (갭 레이어는 7/7 정상).
      · 정의 자체는 의도된 것이다 — '확장 대상 아님'을 0으로 표현한 것.
        문제는 그 사실이 색 인코딩으로 전달되지 않는 것이다
      · 방향: `priority = 0`을 순차 램프에서 빼 중립색 + 범례 별도 칸("확장 대상 아님")으로
        두고, 남은 37개에 7분위를 다시 계산한다. 화면에 이미 `데이터 없음`(`--seq-null`)이라는
        같은 성격의 처리가 있다
      · **Sprint 2 검증의 빈틈이기도 하다**: "램프 7단계가 실제로 다 쓰인다"를
        기본 레이어(갭)에서만 확인했다. 레이어를 바꿔 가며 보지 않았다
      · 색 인코딩 변경이라 `DESIGN.md` 규칙에 걸린다 — 고치면 §2.2에 근거를 남긴다
