...
- [ ] Overpass 공개 인스턴스가 504·429를 자주 낸다. `kumi.systems` 폴백을 넣었지만
      월간 CI에서 재시도 간격을 한 번 더 볼 것
...
- [ ] 패널 제목의 `letter-spacing: 0.11em` + `uppercase`는 라틴 기준 규격이라
      한글에서 자간이 어색하게 벌어진다 (`갭  점수`). DESIGN.md §5를 한글에 맞게
      손볼지 결정 필요 — 지금은 스펙대로 두었다
